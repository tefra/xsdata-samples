...
class ManifestApiEntryBase:
    ...
    :ivar admin_notes: Additional information provided by host
        developers for the client developers.  E.g. "We are currently
        not delivering &lt;description&gt; elements because our model is
...
class HttpwithOptionalLang:
    ...
    :ivar lang: If given, it contains the language code the client
        should expect the website content to be in.  Also see comments
        on xml:lang attribute in StringWithOptionalLang type.
    """
    class Meta:
...
class StringWithOptionalLang:
    ...
    :ivar lang: A note for client developers: Keep in mind, that
        ...
        https://www.ietf.org/rfc/bcp/bcp47.txt  You can also discuss
        this tag in context of EWP here: https://github.com/erasmus-
        without-paper/ewp-specs-architecture/issues/11
    """
    value: Optional[str] = field(
        default=None,
        metadata={
...
class ErrorResponse:
    ...
    :ivar developer_message: A message for the client developer.  In
        case of client errors (HTTP 4xx responses) it should describe
        what the client did wrong (e.g. a required parameter is missing,
    ...
    :ivar user_message: A message for the client user.  This message
        will usually be provided only in case of some very specific
        client errors (HTTP 4xx responses) - the ones that the client
        actually *expects* to happen sometimes. For example, if the
        specification explicitly states that some parameter values can
        be rejected by the server, but it doesn't state which ones
        (because it's up to the server implementers to decide).  In such
        cases, the "wrong" values are usually provided by the end user.
        And this error message should contain the explanation why the
        server believes they're "wrong". For example:  '''University of
        Warsaw doesn't currently allow receiving institutions to propose
        changes to the content of the 'Recognition at the Sending
        Institution' table. Please contact the sending coordinator by
        email to propose this change.'''  User messages MAY be provided
        in multiple languages. English SHOULD be among them.
    """
    class Meta:
        name = "error-response"
        namespace = "https://github.com/erasmus-without-paper/ewp-specs-architecture/blob/stable-v1/common-types.xsd"
...
class SuccessUserMessage(MultilineStringWithOptionalLang):
    """This element is sometimes added to regular HTTP 200 responses. This
    serves as an equivalent of the `user-message` element introduced in the
    `error-response` above, but this one is for HTTP 200 responses (not error
    responses).
