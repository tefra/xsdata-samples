...
class LogAndTraceInstantiation:
    """
    This meta-class defines the attributes for the Log&amp;Trace configuration on a
    specific machine.
    ...
    :ivar dlt_log_channels: DltLogChannels that are configured for the
        log/trace message output
    :ivar queue_size: Length of the queue (in which messages can be
        stored before processing) in the unit "Log message".
    :ivar session_id_support: This attribute defines whether the
        sessionId is used or not.
    :ivar time_base_resource_ref: This reference is used to describe to
        which time base the Log and Trace module has access. From the
        Time Base Resource the Log and Trace module gets the needed
        information to generate the time stamp.
    ...
    class TimeBaseResourceRef(Ref):
        dest: Optional[TimeBaseResourceSubtypesEnum] = field(
            default=None,
            metadata={
                "name": "DEST",
                "type": "Attribute",
