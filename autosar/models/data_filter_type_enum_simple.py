...
class DataFilterTypeEnumSimple(Enum):
    ...
    :cvar MASKED_NEW_DIFFERS_MASKED_OLD: Pass messages where the masked
        ...
        new_value: current value of the message  old_value: last value
        of the message (initialized with the initial value of the
        message, updated with new_value if the new message value is not
        filtered out)
    :cvar MASKED_NEW_DIFFERS_X: Pass messages whose masked value is not
        equal to a specific value x  (new_value&amp;mask) != x
