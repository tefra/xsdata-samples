...
class AdxpExplicit:
    ...
    defined regionally, nationally, or on an enterprise level (e.g. in
    military addresses). Addresses are usually broken up into lines, which
    are indicated by special line-breaking delimiter elements (e.g., DEL).
...
class AnyType:
    ...
    value without belonging to any concrete type. Every concrete type is a
...
class EnxpExplicit:
    ...
    entity name, and a qualifier code for more detail about the name part
    type. Typical name parts for person names are given names, and family
    names, titles, etc.
...
class Bin(AnyType):
    ...
    Binary data is a protected type that MUST not be used outside the data
    type specification.
...
class Ii(AnyType):
    ...
    Number (VIN), etc. Instance identifiers are defined based on ISO object
    identifiers.
...
class Qty(AnyType):
    ...
    has an order relation (less-or-equal) and (2) where difference is
    defined in all of the data type's totally ordered value subsets.
...
class TelExplicit:
    ...
    The address is specified as a URL qualified by time specification and
    use codes that help in deciding which address to use for a given time
    and purpose.
...
class AdExplicit:
    ...
    A sequence of address parts, such as street or post office Box, city,
...
class Ed(Bin):
    ...
    (e.g., XML-signatures.) Instead of the data itself, an ED may contain
...
class Mo(Qty):
    ...
    Currencies are the units in which monetary amounts are denominated in
    different economic regions. While the monetary amount is a single kind
    of quantity (money) the exchange rates between the different units are
    variable. This is the principle difference between physical quantity
    and monetary amounts, and the reason why currency units are not
    physical units.
...
class Real(Qty):
    ...
    the number of significant decimal digits is known as the precision.
    Real numbers are needed beyond integers whenever quantities of the real
    world are measured, estimated, or computed from other real numbers. The
    term "Real number" in this specification is used to mean that
    fractional values are covered without necessarily implying the full set
    of the mathematical real numbers.
    """
...
class ThumbnailExplicit:
    ...
    A thumbnail requires significantly fewer resources than the full data,
    while still maintaining some distinctive similarity with the full data.
    A thumbnail is typically used with by-reference encapsulated data. It
...
class Cd(AnyType):
    ...
    qualifier "LEFT". In exceptional cases, the concept descriptor need not
...
class St(Ed):
    ...
    for machine processing (e.g., sorting, querying, indexing, etc.) Used
